...
const kCookieStoreUrl = Symbol('CookieStore url')
...
class CookieStore extends EventTarget {
  constructor () {
    ...
    this[kCookieStore] = []
    this[kCookieStoreUrl] = arguments[2] ? new URL(arguments[2]) : null
  ...
  get (name = {}) {
    ...
    if (typeof name === 'object') {
      const options = webidl.converters.CookieStoreGetOptions(name)
      ...
      const list = queryCookies(this[kCookieStore], options.name)
  ...
  set (name, value = undefined) {
    ...
    if (typeof name === 'object') {
      const options = webidl.converters.CookieInit(name)
      ...
      const r = setCookie(options, this[kCookieStoreUrl])
      ...
      // Note: duplicate cookies are still stored, but sorted by their
      // path length or creation time.
      ...
      const r = setCookie({ name, value }, this[kCookieStoreUrl])
  ...
  delete (name) {
    ...
    if (typeof name === 'object') {
      name = webidl.converters.CookieStoreDeleteOptions(name)
    } else {
      name = webidl.converters.USVString(name)
    }
    ...
    const r = deleteCookie(name.name ?? name, this[kCookieStore])
  ...
  [inspect.custom] () {
    return `CookieStore ${inspect(this[kCookieStore])}`
  }
}
 ...
 * @param {string} url the url the cookies come from
 * @returns {CookieStore}
 */
function CookieStoreFrom (cookies, url) {
  const parsedList = []
  ...
  return new CookieStore(kCookieStoreConstructable, parsedList, url)
}
...
webidl.converters.CookieStoreGetOptions = webidl.dictionaryConverter([
  {
...
webidl.converters.CookieInit = webidl.dictionaryConverter([
  {
    converter: webidl.converters.USVString,
    key: 'name',
    required: true
  ...
  {
    // https://www.w3.org/TR/hr-time-3/#dom-epochtimestamp
...
webidl.converters.CookieStoreDeleteOptions = webidl.dictionaryConverter([
  {
    converter: webidl.converters.USVString,
    key: 'name',
    required: true
